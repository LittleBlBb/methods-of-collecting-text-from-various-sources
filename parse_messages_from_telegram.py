...
class TelegramApiHandler:
    async def ask_input(self, root, title, prompt, show=None):
    ...
    async def telegram_connect(self, api_id, api_hash, root):
        client = TelegramClient('session', api_id, api_hash)
        try:
            await client.connect()
            if not await client.is_user_authorized():
                while True:
                    phone = await self.ask_input(root, "Телефон", "Введите номер телефона:")
                    if phone:
                        break
                    root.after(0, lambda: simpledialog.messagebox.showwarning("Предупреждение", "Номер телефона обязателен!"))
                await client.send_code_request(phone)
                while True:
                    code = await self.ask_input(root, "Код", "Введите код из Telegram:")
                    if code:
                        break
                ...
                try:
                    ...
                    if "Two-steps verification" in str(e):
                        while True:
                            password = await self.ask_input(root, "Пароль", "Введите пароль:", show="*")
                            if password:
                                break
    ...
    async def fetch_chat_history(self, client, chat_name, update_progress=None):
        if not client:
