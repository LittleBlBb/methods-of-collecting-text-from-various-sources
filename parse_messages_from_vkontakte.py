...
async def get_full_conversation(user_id, access_token, update_progress=None):
    offset = 0
    messages = []
    total_messages = None
...
async def get_user_groups(user_id, access_token):
    try:
        async with aiohttp.ClientSession() as session:
...
async def get_user_posts_in_group_limited(user_id, group_id, access_token, limit=100):
    try:
        async with aiohttp.ClientSession() as session:
...
async def get_user_posts_in_group_full(user_id, group_id, access_token):
    posts = []
    offset = 0
...
async def get_user_posts_in_groups_limited(user_id, access_token, limit_per_group=100, update_progress=None):
    groups = await get_user_groups(user_id, access_token)
    all_posts = []

    for i, (group_id, group_name) in enumerate(groups):
        logger.info(f"Собираем последние посты из группы: {group_name}")
        group_posts = await get_user_posts_in_group_limited(user_id, group_id, access_token, limit_per_group)
...
async def get_user_posts_in_groups_full(user_id, access_token, update_progress=None):
    groups = await get_user_groups(user_id, access_token)
    all_posts = []

    for i, (group_id, group_name) in enumerate(groups):
        logger.info(f"Собираем все посты из группы: {group_name}")
        group_posts = await get_user_posts_in_group_full(user_id, group_id, access_token)
        for post in group_posts:
            post['group_name'] = group_name
        all_posts.extend(group_posts)
        if update_progress:
